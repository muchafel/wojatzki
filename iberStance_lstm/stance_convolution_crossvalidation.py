...
from keras.layers import Dense, Embedding, TimeDistributed, Bidirectional, Flatten, Dropout, MaxPooling1D, GlobalMaxPooling1D, Convolution1D
...
for file in leave_out_Files:
    ...
    words.add(Convolution1D(nb_filter=nb_filter,filter_length=filter_length,border_mode='valid', activation='relu',subsample_length=1))

    # we use max over time pooling:
    words.add(GlobalMaxPooling1D())
    ...
    for i in range(0, len(predictions)):
        ...
        with open("result/cv3/CONV_"+language+"_activation_"+str(activation)+"_opimizer"+str(optimizer)+"_filterLength_"+str(filter_length)+"result_dropout_"+str(dropout)+"_epochs_numberEpochs"+str(numberEpochs)+'_lossFunction_'+str(lossFunction)+"_plusDropout_.txt", "a+") as file:
                                file.write(str(gold)+"\t"+str(predicted)+"\n")

        with open("result/cv3/CONV_"+language+"_activation_"+str(activation)+"_opimizer"+str(optimizer)+"_filterLength_"+str(filter_length)+"result_dropout_"+str(dropout)+"_epochs_numberEpochs"+str(numberEpochs)+'_lossFunction_'+str(lossFunction)+"_plusDropout_id2Outcome.txt", "a+") as file2:
