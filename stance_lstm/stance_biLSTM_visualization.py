    ...
    for file in files:
        ...
        for line in open(file):
            ...
            for idx in xrange(0, min(maxSentenceLen, len(tokens))):
                wordId = getWordIdx(tokens[idx], word2Idx)
                tokenIds[idx] = wordId
...
words.compile(loss='categorical_crossentropy',optimizer='adadelta',metrics=['accuracy'])
#words.compile(loss='categorical_crossentropy',optimizer='adam',metrics=['accuracy'])
words.summary()

plot(words, to_file='network_architecture_wo_names.png',show_shapes=True,show_layer_names=False)