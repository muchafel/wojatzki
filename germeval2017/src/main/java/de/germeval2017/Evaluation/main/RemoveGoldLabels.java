...
//		SentimentDocumentSet set =reader.read(new File("/Users/michael/Desktop/germeval_data_final_v1.3/test.xml"));
		SentimentDocumentSet set =reader.read(new File("/Users/michael/Desktop/germeval_data_final_v1.3/test_diachron_gold.xml"));
	...
	private static void write(SentimentDocumentSet set, boolean writeXml) throws JAXBException, IOException {
		...
		if(writeXml){
	        ...
	        // Write to File
	        m.marshal(set, new File("/Users/michael/Desktop/germeval_data_final_v1.3/test_diachron_clear.xml"));
		}else{
	...
	private static SentimentDocumentSet removeGold(SentimentDocumentSet set) {
		for(SentimentDocument doc:set.getDocs()){
			doc.setAspects(null);
			doc.setSentiment("unknown");
