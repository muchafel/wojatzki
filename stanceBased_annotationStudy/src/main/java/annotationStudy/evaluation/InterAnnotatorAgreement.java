...
//		agreementCalculator.calculateIAA(baseDir + "/semevalTask6/annotationStudy/curatedTweets/Atheism/all");
		agreementCalculator.calculateTotalIAA(baseDir + "/semevalTask6/annotationStudy/curatedTweets/Atheism/all");
	}
...
//		System.out.println(target);
	...
	private String getAnnotatorDecisison(JCas jcas, String annotator) {
		...
		for(String subTarget:subTargets){
			for(SubTarget target: JCasUtil.select(jcas,SubTarget.class)){
				if(target.getAnnotator().equals(annotator) && target.getTarget().equals(subTarget) && !target.getPolarity().equals("NONE")){
	...
	private void calculateIAA(String path) throws Exception {
...
//		}
		...
		FleissKappaAgreement fleissKappa = new FleissKappaAgreement(study);
//		System.out.println("pa: "+pa.calculateAgreement()+ " fleiss "+fleissKappa.calculateAgreement());
		String toPrint=String.valueOf(fd.getCount(target+"_FAVOR"))+"\t"+String.valueOf(fd.getCount(target+"AGAINST"))+"\t"+String.valueOf(fd.getCount(target+"_NONE"));
...
//				System.out.println(annotator+": "+subtarget.getTarget()+"_"+subtarget.getPolarity());
