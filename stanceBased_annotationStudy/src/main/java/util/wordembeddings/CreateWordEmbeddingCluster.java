...
/**
 * this class is used to create clusters from wordembedding files
 * configure the class by the corresponding wordEmbeddingClusteringConfig.xml
 * @author michael
 *
 */
public class CreateWordEmbeddingCluster {

	public static void main(String[] args) throws Exception {

		File file = new File("src/main/resources/wordEmbeddingClusteringConfig.xml");
		JAXBContext jaxbContext = JAXBContext.newInstance(GloVeClusteringConfig.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		GloVeClusteringConfig config = (GloVeClusteringConfig) jaxbUnmarshaller.unmarshal(file);
		
		int numberOfClusters = config.getNumberOfClusters();
		String embeddingsLocation = config.getEmbeddingsLocation();
		boolean writeARFF = config.isWriteARFF();
		int embeddingsDimensions = config.getEmbeddingsDimensions();
		String arffLocation = config.getArffLocation();
		int maxIterations = config.getMaxIterations();
		String centroidLocation = config.getCentroidLocation();
		String clusterIdToWordLocation = config.getClusterIdToWordLocation();
		String wordToCluserLocation = config.getWordToCluserLocation();
		...
		try {
			wordVectors = WordVectorSerializer.loadTxtVectors(new File(embeddingsLocation));
		...
		//
		// INDArray
		// vec1=wordVectors.getWordVectorsMean(Arrays.asList(sentence1.split(" ")));
...
//			System.out.println(centroidVec);
		...
		Map<Integer, List<String>> clusterIdToWords = new HashMap<Integer, List<String>>();
		Map<String, Integer> wordToCluster = new HashMap<String, Integer>();

		int counter = 0;
		for (int assignment : kMeans.getAssignments()) {
			// System.out.println(counter+" ("+wordVectors.lookupTable().getVocabCache().elementAtIndex(counter).getLabel()+") : "+assignment);
			clusterIdToWords = addAssignement(assignment,wordVectors.lookupTable().getVocabCache().elementAtIndex(counter).getLabel(), clusterIdToWords);
			wordToCluster.put(wordVectors.lookupTable().getVocabCache().elementAtIndex(counter).getLabel(), assignment);
			counter++;
		}
		System.out.println("print assignments to " + clusterIdToWordLocation + " and " + wordToCluserLocation);
		printClustersToWords(clusterIdToWordLocation, clusterIdToWords);
		printWordsToClusters(wordToCluserLocation, wordToCluster);
	}

	private static void printWordsToClusters(String clusterIdToWordLocation, Map<String, Integer> wordToCluster)
			throws IOException {
		for (String word : wordToCluster.keySet()) {
			FileUtils.write(new File(clusterIdToWordLocation), word + " > " + wordToCluster.get(word) + "\n", "UTF-8",
					true);
		}
	}
	...
	private static void printClustersToWords(String clusterIdToWordLocation,
		...
		for (int id : clusterIdToWords.keySet()) {
			FileUtils.write(new File(clusterIdToWordLocation), id + " > " + clusterIdToWords.get(id) + "\n", "UTF-8",
					true);
		}
	}

	private static void printCentroids(Map<Integer, INDArray> centroidVectors, String centroidLocation)
			throws IOException {
		for (int id : centroidVectors.keySet()) {
			FileUtils.write(new File(centroidLocation), id + " > " + centroidVectors.get(id) + "\n", "UTF-8", true);
		}
	}

	private static Map<Integer, List<String>> addAssignement(int assignment, String label,
	...
	private static File writeARFF(File embeddingsFile, int dimensions, String arffLocation) throws IOException {
