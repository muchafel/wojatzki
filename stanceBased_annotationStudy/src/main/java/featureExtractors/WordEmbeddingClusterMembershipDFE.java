...
public class WordEmbeddingClusterMembershipDFE extends FeatureExtractorResource_ImplBase implements FeatureExtractor {
	...
	private File wordToClusterFile;
	...
	public boolean initialize(ResourceSpecifier aSpecifier, Map aAdditionalParams)
		...
		try {
			System.out.println("+++ " + wordToClusterFile);
	...
	private Map<String, String> readMappingFromFile(File wordToClusterFile) throws IOException {
		Map<String, String> mapping = new HashMap<String, String>();
		System.out.println(wordToClusterFile.getAbsolutePath());
		List<String> lines = FileUtils.readLines(wordToClusterFile, "UTF-8");
	...
	public Set<Feature> extract(JCas jcas, TextClassificationTarget target) throws TextClassificationException {
		...
		for (Token t : JCasUtil.select(jcas, Token.class)) {
			...
			if (wordToCluster.containsKey(tokenText)) {
				...
				// System.out.println(tokenText+" contained in cluster"+
				// wordToCluster.get(tokenText));
			}
		}
