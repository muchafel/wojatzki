...
//			System.out.println("number of instances "+map.size());
	...
	public void getNext(JCas jcas) throws IOException, CollectionException {
		...
		try {
			createMetaData(jcas);
			jcas.setDocumentText(getText1()+"_"+getText2());
...
//		System.out.println(lines.get(rowCounter).split("\t")[columnCounter]);
	...
	protected void createMetaData(JCas jcas) {
		DocumentMetaData metaData= new DocumentMetaData(jcas);
//		 DocumentMetaData metaData = DocumentMetaData.create(jcas);
	        try {
				metaData.setDocumentTitle(getText1()+"_"+getText2());
			} catch (TextClassificationException e) {
				e.printStackTrace();
			}
	        try {
				metaData.setDocumentId(getTitle1() + " " + getTitle2());
			} catch (TextClassificationException e) {
				e.printStackTrace();
			}
