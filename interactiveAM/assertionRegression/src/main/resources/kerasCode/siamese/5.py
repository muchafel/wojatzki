...
def runExperiment(seed, trainVec, trainOutcome, testVec, testOutcome, embedding, maximumLength, predictionOut):
	...
	#model.add(Merge([left, right], mode = L1_distance, output_shape=lambda x: x[0]))
	model.add(Merge(mode=euclideanSqDistance, output_shape=lambda x: (x[0][0], 1), name='distanceMerge') ([left, right]))
	model.add(Dense(1, kernel_initializer='normal'))	
